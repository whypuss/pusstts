...
class GeneratedAudio(
    ...
    fun save(filename: String): Boolean {
        return try {
            val file = java.io.RandomAccessFile(filename, "rw")
            val dataSize = samples.size * 2  // 16-bit PCM
            ...
            // fmt chunk
            file.writeBytes("fmt ")
            file.writeInt(Integer.reverseBytes(16))          // chunk size
            file.writeShort(java.lang.Short.reverseBytes(1.toShort()).toInt()) // PCM format
            file.writeShort(java.lang.Short.reverseBytes(1.toShort()).toInt()) // mono
            file.writeInt(Integer.reverseBytes(sampleRate))   // sample rate
            file.writeInt(Integer.reverseBytes(sampleRate * 2)) // byte rate
            file.writeShort(java.lang.Short.reverseBytes(2.toShort()).toInt()) // block align
            file.writeShort(java.lang.Short.reverseBytes(16.toShort()).toInt()) // bits per sample
